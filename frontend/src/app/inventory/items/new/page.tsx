...
export default function NewItemPage() {
  ...
  const loadSuppliers = async () => {
    try { setSuppliers(await api.get<Supplier[]>("/inventory/suppliers/")); } catch { }
  };

  const handleSubmit = async (data: Parameters<typeof api.post>[1]) => {
    setSubmitting(true); setError("");
    try {
      const created = await api.post<{ id: string }>("/inventory/items/", data);
      router.push(`/inventory/items/${created.id}`);
    } catch (err) {
      setError(err instanceof ApiRequestError ? err.message : "Failed to create item.");
  ...
  return (
    <DashboardShell user={user} onLogout={logout}>
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <ItemForm suppliers={suppliers} onSubmit={handleSubmit} loading={submitting} />
      </div>
    </DashboardShell>
  );
}
